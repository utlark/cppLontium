...
I2cDevice::I2cDevice (const std::string &device, const int addr) : _addr(addr)
{
...
std::vector<uint8_t> I2cDevice::ReadRegs (uint8_t reg, const size_t len) const
{
...
uint8_t I2cDevice::ReadReg (const uint8_t reg) const
{
    return ReadRegs(reg, 1)[0];
}

void I2cDevice::WriteRegs (const uint8_t reg, const std::vector<uint8_t> &values) const
{
...
void I2cDevice::WriteReg (const uint8_t reg, uint8_t value) const
{
    WriteRegs(reg, { value });
}